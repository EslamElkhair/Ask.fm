...
Question() :
	...
	Question(const std::string &line) {
		std::vector<std::string> substrs = help.SplitString(line);
		assert(substrs.size() == 7);

		question_id = help.ToInt(substrs[0]);
		parent_question_id = help.ToInt(substrs[1]);
		from_user_id = help.ToInt(substrs[2]);
		to_user_id = help.ToInt(substrs[3]);
		is_anonymous_questions = help.ToInt(substrs[4]);
...
public:
	...
	User(const std::string &line) {
		...
		user_id = help.ToInt(substrs[0]);
		user_name = substrs[1];
		password = substrs[2];
		name = substrs[3];
		email = substrs[4];
		allow_anonymous_questions = help.ToInt(substrs[5]);
	}
...
public:
	...
	void LoadDatabase() {
		...
		std::vector<std::string> lines = help.ReadFileLines("questions.txt");
	...
	void UpdateDatabase() {
		...
		help.WriteFileLines("questions.txt", lines, false);
	}
};
...
public:
	...
	void LoadDatabase() {
		...
		std::vector<std::string> lines = help.ReadFileLines("users.txt");
		for (const auto &line : lines) {
			User user(line);
	...
	void AccessSystem() {
		int choice = help.ShowReadMenu( { "Login", "Sign Up" });
		if (choice == 1)
			DoLogin();
		else
			DoSignUp();
	...
	void UpdateDatabase(const User &user) {
		...
		help.WriteFileLines("users.txt", lines);
	}
...
public:
	void Run() {	// only public one
		...
		while (true) {
			int choice = help.ShowReadMenu(menu);
			LoadDatabase(true);
